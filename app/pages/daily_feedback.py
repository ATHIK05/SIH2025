...
def show_daily_feedback_entry():
    st.subheader('📅 Enter Daily Feedback for Student')
    ...
    class_name = st.text_input('Class (e.g., 10A, 12B, etc.)', key='daily_class_name')
    ...
    student_display = st.selectbox('Student', list(student_choices.keys()), key='daily_student_select') if students else None
    date = st.date_input('Date', value=datetime.today(), key='daily_date')
    if student_display:
        regno = student_choices[student_display]
        feedback = st.text_area('Daily Feedback (optional)', key='daily_feedback')
        if st.button('Submit Daily Feedback', key='daily_submit_btn'):
            if not feedback:
